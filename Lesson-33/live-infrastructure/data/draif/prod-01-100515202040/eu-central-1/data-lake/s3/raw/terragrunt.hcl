...
locals {
  ...
  s3_prefix       = "${local.resource_prefix}-${local.layer}"
}
...
inputs = {
  ...
  items = {
    ...
    branch-onboarding-retail = {
      bucket = "${local.s3_prefix}-branch-onboarding-retail"
      tags   = merge(local.tags_map, { ITOwner01 = "oleksandr.holubenko@raiffeisen.ua" })
    }
    calc = {
    ...
    gdpr-zone = {
      bucket = "${local.s3_prefix}-gdpr-zone"
      tags   = merge(local.tags_map, { ITOwner01 = "oleksandr.holubenko@raiffeisen.ua" })
    }
    int = {
    ...
    onboarding-backend = {
      bucket = "${local.s3_prefix}-onboarding-backend"
      tags   = merge(local.tags_map, { ITOwner01 = "oleksandr.holubenko@raiffeisen.ua" })
    }
    fasttack = {
      bucket = "${local.s3_prefix}-fasttack"
      tags   = merge(local.tags_map, { ITOwner01 = "oleksandr.holubenko@raiffeisen.ua" })
    }
    leauth = {
      bucket = "${local.s3_prefix}-leauth"
      tags   = merge(local.tags_map, { 
        "business:product-owner"      = "maksym.gierasimov@raiffeisen.ua"
        "business:emergency-contact"  = "sergii.leschenko@raiffeisen.ua"
        "business:cost-center"        = "0669"
        product                       = "LEAUTH"
        })
    }
  }
