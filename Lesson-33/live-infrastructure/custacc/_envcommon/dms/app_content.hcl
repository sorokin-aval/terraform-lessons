...
# Include EFS
dependency "efs" { config_path = find_in_parent_folders("efs/dms-share") }
...
inputs = {
  ...
  user_data                    = join("" , [local.user_data_locals.locals.user_data_header, "project='${lower(local.tags_map.locals.tags["business:product-project"])}'\nname='${local.name}'\ndevice_name='/dev/sdh'\n\n", local.user_data_locals.locals.user_data_body ], [ "mount_point='${dependency.efs.outputs.mount_target_ips["0"]}'\nmount_path='/opt/${lower(local.tags_map.locals.tags["business:product-project"])}_content'\n\n", local.user_data_locals.locals.user_data_efs_mount])
