...
dependency "eks_role" {
  config_path  = "../iam/iam-role-for-service-accounts-eks/sa/"
  mock_outputs = {
    iam_role_arn = "arn:aws:iam::${local.aws_account_id}:role/mock_role_id"
  }
  mock_outputs_allowed_terraform_commands = ["plan"]
}
dependencies {
  paths = ["../iam/iam_assumable_role_with_saml/team/"]
}
  ...
  kms_policy                  = <<EOF
{
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Effect": "Allow",
            "Principal": {
                "AWS": ["arn:aws:iam::${local.aws_account_id}:role/${local.tags_map.Nwu}-${local.tags_map.Tech_domain}-team","arn:aws:iam::${local.aws_account_id}:role/${local.tags_map.Nwu}-${local.tags_map.Tech_domain}-sa"]
            },
            "Action": [
                "kms:Encrypt",
                "kms:Decrypt",
...
EOF
  airflow_configuration       = {
    ...
    "idp_client_id" : "${local.tags_map.Nwu}-${local.tags_map.Domain}-${local.tags_map.Project}"
  ...
  vault_path                   = "secrets/service-internal-secrets/${local.tags_map.Environment}/${local.tags_map.Tech_domain}"
}

terraform {
  source = "git::https://code.rbi.tech/raiffeisen/ua-tf-data-analytics-platform//data-analytics-platform?ref=data-analytics-platform_v1.0.0-alpha"
}
