...
dependency "zone" { config_path = find_in_parent_folders("core-infrastructure/route53-zone") }
...
inputs = {
  ...
  alb_route53_records = [local.app_vars.locals.name],
  hosted_zone         = lookup(dependency.zone.outputs.route53_zone_zone_id, local.account_vars.locals.domain),
}
