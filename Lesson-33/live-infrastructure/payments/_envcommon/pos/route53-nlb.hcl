...
dependency "nlb-external" {
  config_path = find_in_parent_folders("nlb-external")
}
  ...
  records = jsonencode([
    {
      ...
      type = "A"
      alias = {
        name                   = dependency.nlb-external.outputs.lb_dns_name
        zone_id                = dependency.nlb-external.outputs.lb_zone_id
