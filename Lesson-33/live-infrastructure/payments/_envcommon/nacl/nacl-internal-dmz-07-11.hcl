...
inputs = {
  ...
  inbound_acl_rules = concat(flatten([ for bnum, cidr_blocks in {
      ...
      "700" = local.account_vars.locals.ips["ibm-mb"],
      "800" = local.account_vars.locals.ips["ad-test"],
      "900" = local.account_vars.locals.ips["aval-common-test"],
      "1000" = local.account_vars.locals.ips["comm-vault"],
      "1100" = local.account_vars.locals.ips["jump"],
    } :
