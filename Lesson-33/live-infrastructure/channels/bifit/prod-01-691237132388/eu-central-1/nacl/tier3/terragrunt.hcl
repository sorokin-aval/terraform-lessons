...
locals {
  name         = "NACL-RBUA-${local.account_vars.environment_letter}-${local.account_vars.tier3_subnet_abbr}"
...
inputs = {
  name        = local.name
  description = "NACL for the Tier3 subnets"
  ...
  inbound_acl_rules = concat(
    ...
    # Allow inbound tcp access from RBUA on-premise private subnets
    [ for i, subnet in local.account_vars.user_private_subnets_list :
      {
        rule_number = 900 + i
        rule_action = "allow"
        from_port   = 443
  ...
  outbound_acl_rules = concat(
    ...
    # Allows outbound traffic to VDI user pool
    [ for i, subnet in local.account_vars.user_private_subnets_list :
      {
        rule_number = 1200 + i
        rule_action = "allow"
