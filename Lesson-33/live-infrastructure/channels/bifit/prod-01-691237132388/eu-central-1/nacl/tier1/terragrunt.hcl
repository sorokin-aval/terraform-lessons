...
inputs = {
  ...
  inbound_acl_rules = concat(
    ...
    # Allow inbound HTTPS access from anywhere (restrict access only from CF subnets in SG)
    [ 
      {
        rule_number = 600
        rule_action = "allow"
    ...
    # Allows inbound traffic to HAProxy stats page from internal networks
    [ 
      {
        rule_number = 1300
        rule_action = "allow"
