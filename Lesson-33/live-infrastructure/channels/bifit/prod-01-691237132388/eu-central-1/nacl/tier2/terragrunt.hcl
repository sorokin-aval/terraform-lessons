...
inputs = {
  ...
  inbound_acl_rules = concat(
    ...
    # Allows inbound access to applications from technology private IP pool
    [ for i, subnet in local.account_vars.tech_support_subnets_list :
      {
        rule_number = 900 + i
        rule_action = "allow"
        from_port   = 9633
    ...
    # Allows inbound access to applications from support private IP pool
    [ for i, subnet in local.account_vars.support_access_subnets_list :
      {
        rule_number = 1000 + i
        rule_action = "allow"
    ...
    # Allow inbound tcp access from RBUA AWS private subnets
    [ for i, subnet in local.account_vars.rbua_private_aws_subnets_list :
      {
        rule_number = 1200 + i
        rule_action = "allow"
  ...
  outbound_acl_rules = concat(
    ...
    # Allows outbound HTTPS connections to infra subnets
