...
locals {
  name         = "SG-RBUA-${local.account_vars.environment_letter}-LIAInstanceADM"
...
inputs = {
  name        = local.name
  description = "Security group for the ADM instance"
  ...
  ingress_with_cidr_blocks = concat(
    [
      ...
      },
      {
        ...
        to_port     = 8082
        protocol    = "tcp"
        description = "HTTPS-APP access from tier2 subnets"
        cidr_blocks = local.account_vars.tier2_subnets
      },
      {
        name        = "HTTPS-APP OIM"
        ...
        to_port     = 8082
        protocol    = "tcp"
        description = "HTTPS-APP access from OIM subnets"
        cidr_blocks = local.account_vars.oim_subnets
      },
      {
        name        = "SSH - Ansible"
        from_port   = 22
    ...
    [ for subnet in local.account_vars.inbound_adm_is_subnets_list :
      {
        from_port   = 8082
        to_port     = 8082
        protocol    = "tcp"
  ...
  egress_with_cidr_blocks = [
    ...
    {
      name        = "On-Premise AD - Kerberos TCP"
      from_port   = 88
      to_port     = 88
      protocol    = "tcp"
      description = "Access to Active Directory On-Premise - Kerberos TCP"
      cidr_blocks = local.account_vars.ad_onprem_subnets
    },
    {
      name        = "On-Premise AD - Kerberos UDP"
      from_port   = 88
      to_port     = 88
      protocol    = "udp"
      description = "Access to Active Directory On-Premise - Kerberos UDP"
      cidr_blocks = local.account_vars.ad_onprem_subnets
    },
    {
      name        = "RO OTP"
