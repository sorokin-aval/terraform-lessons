dependency "ami_tags_management" {
  config_path = find_in_parent_folders("iam/policy/ami-tags-management")
}

dependency "ami_create" {
  config_path = find_in_parent_folders("iam/policy/ami-create")
}

dependency "s3_policy_exchange" {
  config_path = find_in_parent_folders("iam/policy/s3-exchange")
}

dependency "ssm_policy" {
  config_path = find_in_parent_folders("iam/policy/ssm")
}

dependency "secret_manager_policy_app_read_write" {
  config_path = find_in_parent_folders("iam/policy/secret-manager-app-read-write")
}
