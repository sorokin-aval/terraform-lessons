...
dependency "sg" {
  config_path = find_in_parent_folders("sg/instance-${local.type}")
...
locals {
  ...
  tags_back_first = merge(
    local.tags,
    try(local.account_vars.tag_schedule_is-back_all_first, "") != "" ? { Schedule = local.account_vars.tag_schedule_is-back_all_first } : {}
  )
  tags_back_second = merge(
    local.tags,
    try(local.account_vars.tag_schedule_is-back_all_second, "") != "" ? { Schedule = local.account_vars.tag_schedule_is-back_all_second } : {}
  )
}
