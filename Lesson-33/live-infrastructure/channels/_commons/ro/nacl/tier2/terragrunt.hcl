...
#    ],
    ...
    # Allow inbound HTTPS access(OpenSearch) from RBUA private subnets
    ...
    # Allows access to SMTP VPC Endpoint
    [ for i, subnet in local.account_vars.tier2_subnets_list :
      {
        rule_number = 800 + i
        rule_action = "allow"
...
#    ],
    ...
    # Allows inbound OpenSearch port 443 from common infrastructure subnets
    [ for i, subnet in local.account_vars.common_infra_subnets_list :
      {
        rule_number = 900 + i
        rule_action = "allow"
    ...
    # Allow inbound SSH access for Bastion host
    [ 
      {
        rule_number = 1000
        rule_action = "allow"
  ...
  outbound_acl_rules = concat(
    ...
    # Allows outbound access to SMS GW subnets
    [ for i, subnet in local.account_vars.sms_gw_vpn_subnets_list :
      {
        rule_number = 1600 + i
        rule_action = "allow"
        from_port   = 2777
    ...
    # Allows outbound access to SMPP GW subnets
    [ for i, subnet in local.account_vars.smpp_gw_subnets_list :
      {
        rule_number = 2600 + i
        rule_action = "allow"
        from_port   = 20520
    ...
    # Allows outbound access to SMPP GW subnets
    [ for i, subnet in local.account_vars.kyivstar_gw_vpn_subnets_list :
      {
        rule_number = 2800 + i
        rule_action = "allow"
        from_port   = 80
