...
locals {
  name         = "SG-RBUA-${local.account_vars.environment_letter}-LIAInstanceClientEndpoint"
...
inputs = {
  name        = local.name
  description = "Security group for the ClientEndpoint instance"
  ...
  egress_with_cidr_blocks = concat(
    ...
    [ for subnet in local.account_vars.db_onprem_subnets_list :
      {
        from_port   = 1521
        to_port     = 1521
        protocol    = "tcp"
    ...
    [ for subnet in local.account_vars.robotics_onprem_subnets_list :
      {
        from_port   = 1434
        to_port     = 1434
        protocol    = "tcp"
