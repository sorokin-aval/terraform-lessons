...
dependency "sg" {
  config_path = find_in_parent_folders("sg/instance-console")
}
...
locals {
  name         = "SG-RBUA-${local.account_vars.environment_letter}-LIWALBConsole"
...
inputs = {
  name        = local.name
  description = "Security group for the Console ALB"
  ...
  egress_with_source_security_group_id = [
    {
      ...
      protocol    = "tcp"
      description = "HTTPS access to Console Instance"
