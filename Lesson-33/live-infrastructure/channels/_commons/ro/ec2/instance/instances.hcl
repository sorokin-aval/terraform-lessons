...
#appdyn_conf_path_prefix=$3  # example: '/DBO' or '/opt'
...
java_agent_conf="$appdyn_conf_path_prefix/appdynamics/javaagent/ver/conf/controller-info.xml"
machine_agent_conf="$appdyn_conf_path_prefix/appdynamics/machineagent/conf/controller-info.xml"
machine_agent_analytics_conf="$appdyn_conf_path_prefix/appdynamics/machineagent/monitors/analytics-agent/conf/analytics-agent.properties"

ip=$(ifconfig | sed -En 's/127.0.0.1//;s/.*inet (addr:)?(([0-9]*\.){3}[0-9]*).*/\2/p' | cut -d'.' -f 3,4 | tr '.' '-')

printf -v server_name "%q" $service_name"-"$ip

printf -v node_name "%q" $server_name"-ro.rbua"
